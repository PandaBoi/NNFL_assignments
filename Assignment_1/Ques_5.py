...
def del_j (h_x,X,y,W,idx):
	...
	for i in range(len(y)):
		del_j +=(h_x[i] - y[i])*X[i,idx] + 1/2 * LAMBDA * np.sign(W[idx])
		
	return del_j



def batch_grad(data,alpha = 2e-8,epsilon = 1e-10):
...
def stoch_grad(data,alpha = 2e-8,epsilon = 2e-9):
	...
	while(eps>epsilon) and (ep < epochs):
		...
		for j in range(len(W)):
			...
			W[j] = W[j] - alpha*(del_j_s(h_x,X,y,W,j))
...
for i1,w_1 in enumerate(ww1):
	for i2,w_2 in enumerate(ww2):
		...
		J_cont[i1][i2] = compute_J(h_x, Y,W)
